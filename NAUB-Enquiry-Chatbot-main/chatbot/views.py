...
def _get_or_create_session(request):
    """Return the current ChatSession for this user, fallback to their last saved session, or create one."""
    user = request.user
    session_key = "chat_session_id"
    session_id = request.session.get(session_key)

    # CHECK: Did the user click "New Chat"? If yes, skip history and make a fresh chat!
    if request.session.get("force_new_chat"):
        del request.session["force_new_chat"]  # Clear the flag
        chat_session = ChatSession.objects.create(user=user)
        request.session[session_key] = str(chat_session.session_id)
        return chat_session

    # 1. First, check if the active session key matches a session in the database
    if session_id:
        try:
    ...
    # 2. FIX: If the session cookie expired or got cleared, find the user's LAST active database session
    last_session = ChatSession.objects.filter(user=user).order_by('-started_at').first()
    if last_session:
        request.session[session_key] = str(last_session.session_id)
        return last_session

    # 3. If they are a brand new user with absolutely zero history, create a fresh session
    chat_session = ChatSession.objects.create(user=user)
    request.session[session_key] = str(chat_session.session_id)
    return chat_session
...
def clear_session(request):
    """Clear the current chat session and force a fresh one."""
    # 1. Remove the session key from the browser
    if "chat_session_id" in request.session:
        del request.session["chat_session_id"]
    
    # 2. Tell the system NOT to load the last chat from history this time
    request.session["force_new_chat"] = True
    
    # 3. Send the user back to the clean main page
